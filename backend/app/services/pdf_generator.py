...
class UdyamReport(FPDF):
    ...
    def normalize_text(self, text: str) -> str:
        """Sanitize text for standard Latin-1 Helvetica font."""
        if not text:
            return ""
        replacements = {
            "\u20b9": "Rs. ",
...
class PDFGenerator:
    ...
    def generate(self, report: AnalysisResponse) -> bytes:
        ...
        pdf.section_title("COMPETITOR INTELLIGENCE & MAPPING")
        pdf.key_value("Identified Competitors", f"{report.competitors.count} within {report.radius_km} km")
        pdf.key_value("Competition Density", report.competitors.density)
        if report.competitors.items:
            for item in report.competitors.items[:4]:
                pdf.set_font("Helvetica", "B", 8)
                pdf.cell(50, 5, f"{item.name[:25]}", new_x=XPos.RIGHT)
                pdf.set_font("Helvetica", "", 8)
                pdf.cell(30, 5, f"{item.distance_km} km away", new_x=XPos.RIGHT)
                pdf.cell(0, 5, f"[{item.strength} Strength] {item.offering[:45]}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(3)

        # ── Section 6: Pricing Intelligence ──────────────────────────────────
        ...
        # ── Section 7: Recommendation & Checklist ─────────────────────────────
        ...
        # ── Section 8: Data Sources & Provenance ──────────────────────────────
        if report.data_sources:
            pdf.ln(3)
            pdf.section_title("DATA SOURCES & PROVENANCE")
            pdf.set_font("Helvetica", "", 8)
            for src in report.data_sources:
                pdf.cell(5, 4, "*", new_x=XPos.RIGHT)
                pdf.cell(0, 4, src, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
